...
func attack_player() -> void:
	if not is_instance_valid(player) or current_state == EnemyState.DEAD:
		return
...
func _on_stun_timer_timeout() -> void:
	if current_state == EnemyState.STUNNED:
		transition_to_state(EnemyState.CHASING if is_instance_valid(player) else EnemyState.IDLE)
